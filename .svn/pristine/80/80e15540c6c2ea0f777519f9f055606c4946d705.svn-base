...
var downCounter = function(endHour){
		var downCounterTimer = null;
		var container = document.getElementById("downCounter");
		if(!container){console.error("can't find downCouter dom element!")}
		var eYear = container.getAttribute("endYear"),
		...
		var showDom = function(callback){
			...
			if(callback){
				callback(zeroH + counter.offHour, zeroM + counter.offMin, zeroS + counter.offSec, counter.offMSec);
			}
	 	...
	 	//计算时间差
		var setInitTime = function(callback){
			var endTime = new Date(counter.endYear, counter.endMth, counter.endDay, counter.endHour, counter.endMin, counter.endSec);//结束时间
			console.log([counter.endYear, counter.endMth, counter.endDay, counter.endHour, counter.endMin, counter.endSec]);
			...
			console.log([counter.beginYear, counter.beginMth, counter.beginDay, counter.beginHour, counter.beginMin, counter.beginSec]);
		...
		return {
			init: function(callback){
				//初始化
				setInitTime(callback);

				//开始倒计时
				downCounterTimer = setInterval(function(){
					--counter.offMSec;
						...
						if(counter.offSec < 0){
							...
							if(counter.offMin < 0){
								...
								if(counter.offHour < 0){
									clearInterval(downCounterTimer);
									counter.offSec = 0;
									counter.offMin = 0;
									counter.offHour = 0;
									counter.offMSec = 0;
